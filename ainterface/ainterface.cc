...
bool AbstractInterface::RunMenu(
...
std::size_t AbstractInterface::ShowMenu(const std::string &menu,
                                        const std::size_t items) {
  std::cout << menu;
  return items ? CheckInputItem(-1, items) : 0;
}

[[nodiscard]] std::int64_t AbstractInterface::CheckInputItem(
    const std::int64_t min, const std::int64_t max) {
  std::string line;
  std::getline(std::cin, line);

  std::int64_t result;
  while (!sscanf(line.c_str(), "%ld", &result) || result <= min ||
         result >= max) {
